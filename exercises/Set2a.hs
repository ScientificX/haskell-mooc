...
updateAt :: Int -> a -> [a] -> [a]
updateAt 0 x xs = take 0 xs ++ [x] ++ drop 1 xs
updateAt i x xs = if i == (length xs) then take (i-1) xs ++ [x] ++ drop i xs else take i xs ++ [x] ++ drop (i+1) xs
...
-- data J = String | Char
-- drop :: String -> J
-- drop d = drop 1 (take (length s - 1) s)
-- palindromify :: String -> String
-- palindromify s = if isPalindrome s then s else palindromify drop 1 g
    -- where g = take ((length s) - 1) s
palindromify x = todo
------------------------------------------------------------------------------
...
safeIndex xs i = if i >= length xs then Nothing else (Just $ xs !! i)
...
addEithers (Left str) (Left str1) = (Left str)
addEithers (Right int) (Right int1) = Right (int + int1)

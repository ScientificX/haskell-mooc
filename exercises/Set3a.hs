...
capitalizeFirst str = (map toUpper $ (take 1 str)) ++ (drop 1 str)
...
while check update value = go True value
  where go b x = if (check x) == False then x else go (check x) (update x)
...
whileRight f x = righty f (Right x)
  where righty f (Left b) = b
        righty f (Right a) = righty f (f a)
